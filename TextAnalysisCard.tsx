...
export const TextAnalysisCard: React.FC<TextAnalysisCardProps> = ({ analysis, onPlayAudio, isTtsLoading }) => {
    return (
        <div className="h-full space-y-4">
            {/* Main Translation */}
            <div className="bg-background-tertiary p-3 rounded-lg">
                <div className="flex justify-between items-center mb-1">
                    <span className="text-xs font-semibold text-accent-primary uppercase tracking-wider">Improved Text</span>
                </div>
                <div className="flex justify-between items-start gap-2">
                    <p className="text-lg text-text-primary">{analysis.correctedText}</p>
            ...
            <div className="space-y-4">
                <AnalysisSection icon={<LearningIcon />} title="Key Feedback">
                    <ul className="space-y-2">
                        {analysis.keyFeedbackPoints.map((tip, index) => (
                ...
                <AnalysisSection icon={<MeaningIcon />} title="Meaning & Nuance Analysis">
                    <div className="prose prose-invert prose-sm max-w-none"><ReactMarkdown>{analysis.meaningAnalysis}</ReactMarkdown></div>
                </AnalysisSection>

                <AnalysisSection icon={<StructureIcon />} title="Structure (Grammar & Syntax)">
                    <div className="prose prose-invert prose-sm max-w-none"><ReactMarkdown>{analysis.structureAnalysis}</ReactMarkdown></div>
                </AnalysisSection>

                <AnalysisSection icon={<UsageIcon />} title="Usage (Context & Patterns)">
                    <div className="prose prose-invert prose-sm max-w-none"><ReactMarkdown>{analysis.usageAnalysis}</ReactMarkdown></div>
                </AnalysisSection>
                ...
                <AnalysisSection icon={<AlternativesIcon />} title="Alternative Phrasings">
                    <ul className="list-disc list-inside space-y-1">
                        {analysis.alternativePhrasings.map((alt, index) => (
                            <li key={index}>{alt}</li>
                        ))}
                    </ul>
                </AnalysisSection>
